...
Le mode Refonte devient Radar Rework dans `/radar`, avec un accès direct `/radar/rework`. Il utilise le même compte Supabase que le Radar local et propose des dossiers privés filtrables, l’ajout manuel et la recherche administrative par lots de huit. Les dossiers enregistrés avec un SIREN sont exclus des recherches suivantes ; l’import et l’ajout vérifient aussi le domaine et le nom/commune. Sans identifiant confirmé, les homonymes et changements de raison sociale demandent encore un contrôle humain.
...
Vérifications : typecheck, lint, 37 tests, build Vercel et test du serveur compilé (page Rework, limite de huit, exclusions du carnet Rework, erreur de lecture). Le contrôle Supabase ne signale pas de défaut RLS des nouvelles tables. La vérification du navigateur connecté et de l’envoi réel d’un fichier reste distincte de ces tests. Le navigateur de contrôle ne peut pas joindre le serveur local. Aucun message commercial n’est envoyé et aucune maquette n’est publiée pour une entreprise.
